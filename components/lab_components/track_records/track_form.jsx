...
import { addTrackRecord } from '@/app/lib/lab_actions';
import SaveIcon from '@mui/icons-material/Save';
import { generic_setter } from '@/app/lib/helper';

export default function TrackForm({toggle_function, user, lab, sampleTypes}){

    const [formState, formAction] = useActionState(addTrackRecord, {error:null})
    ...
    useEffect(() => {
        ...
        console.log("Sample: ", selected_sampletypeId)
        console.log("Param: ", selected_parameterId)
        console.log("Method: ", selected_methodId)
        console.log("Ref: ", selected_refId)
    },[selected_methodId,selected_sampletypeId,selected_refId,selected_parameterId])
    ...
    return (
        <>
            <form action={formAction} className={styles.form_style}>
                ...
                <div className={styles.form_row}>
                    <div className={styles.form_item}>
                        <label className={styles.label} htmlFor="num_of_samples">Number of Samples Analyzed:</label>
                        <input className={styles.input} type="number" name="num_of_samples" required/>
                    </div>
                </div>

                <div className={styles.form_row}>
                    <div className={styles.form_item}>
                        <label className={styles.label} htmlFor="datecovered" style={{marginBottom:"10px"}}>Date Covered:</label>
                        <div className={styles.form_row}>
                            <div className={styles.form_item}>
                                <label className={styles.sub_label} htmlFor="start">From:</label>
                                <input className={styles.input} type="date" name="start" required/>
                            </div>
                            <div className={styles.form_item}>
                                <label className={styles.sub_label} htmlFor="end">To:</label>
                                <input className={styles.input} type="date" name="end" required/>
                            </div>
                        </div>
                    </div>
                </div>
